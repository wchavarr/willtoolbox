...
DESCRIPTION: Will Toolbox - Unified Management Platform.
             Includes Local-Only Salesforce configuration logic.
================================================================================
"""
...
try:
    ...
    st.error("🚨 CRITICAL: sf_sync_cli.py not found.")
...
def run_manual_sync():
    ...
    with st.sidebar.status("🔄 Syncing Salesforce...", expanded=True) as status:
        ...
        if conn:
            ...
            st.sidebar.error("CLI Authentication Failed. Check 'sf org list'.")
